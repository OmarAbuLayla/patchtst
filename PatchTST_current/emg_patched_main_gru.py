...
#  Author: Omar A. Layla (adapted with diagnostic fixes)
# ==============================================================
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--epochs-ablation", type=int, default=0, help="Optional override for number of training epochs during quick ablations")
    parser.add_argument("--lr", type=float, default=2.5e-4)
    parser.add_argument("--min-lr", type=float, default=1e-5, help="Lower bound for cosine LR schedule")
    parser.add_argument("--warmup-epochs", type=int, default=0, help="Linear warm-up epochs before cosine decay")
    ...
    parser.add_argument("--patch-dropout", type=float, default=0.0, help="Probability of dropping patches during training")
    parser.add_argument("--stride-test", type=int, default=96, choices=[64, 96, 128], help="Simulated stride for evaluation")
    parser.add_argument("--subset-train", type=int, default=0, help="Optional cap on number of training samples (0 = full set)")
    parser.add_argument("--subset-val", type=int, default=0, help="Optional cap on number of validation samples (0 = full set)")
    parser.add_argument("--subset-test", type=int, default=0, help="Optional cap on number of test samples (0 = full set)")
...
def load_checkpoint(path: str, model: nn.Module, optimizer: Optional[torch.optim.Optimizer] = None) -> int:
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        if log_interval > 0 and (batch_idx % log_interval == 0 or batch_idx == num_batches):
            ...
            print(
                ...
                f"Patches: {patch_mean:.2f}  Time: {elapsed:5.1f}s",
                flush=True,
            )
...
def train_and_evaluate(args: argparse.Namespace) -> None:
    ...
    # Handle Subset wrappers transparently when inspecting dataset metadata.
    ...
    if args.test_only:
        for split_name in ["val", "test"]:
            ...
            if result.topk:
                print(
                    f"{split_name.capitalize()} top-1={result.topk['top1']:.4f} "
                    f"top-5={result.topk['top5']:.4f}"
                )
    ...
    if test_result.topk:
        print(
            f"Test top-1={test_result.topk['top1']:.4f} top-5={test_result.topk['top5']:.4f}"
        )
