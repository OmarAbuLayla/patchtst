# ==============================================================
#  EMG PatchTST GRU Training Script (v5, fixed for on-the-fly patching)
# ==============================================================
...
from emg_patched_dataset5 import build_patch_dataloaders
from emg_patched_model_gru5 import EMG_GRU_PatchTST
...
def create_scheduler(optimizer: torch.optim.Optimizer, *, use_cosine: bool, total_epochs: int,
                     warmup_epochs: int, lr_patience: int, min_lr: float):
    if not use_cosine:
...
def train_and_evaluate(args: argparse.Namespace) -> None:
    ...
    if num_channels is None:
        raise AttributeError("EMGPatchDataset must expose 'num_channels' for model wiring")
    ...
    for epoch_idx in range(start_epoch, total_epochs):
        ...
        if val_result.accuracy > best_val_acc:
            ...
            save_checkpoint(
                {"epoch": epoch_num, "model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args)},
                save_dir,
                "best.pt",
            )
    ...
    *,
    ...
    for batch_idx, (inputs, targets, *_) in enumerate(loader, start=1):
        ...
        if log_interval > 0 and (batch_idx % log_interval == 0 or batch_idx == len(loader)):
            ...
            print(
                f"{desc} [{batch_idx}/{len(loader)}] Loss: {total_loss/total_samples:.4f}  "
                f"Acc: {total_correct/total_samples:.4f}  Time: {elapsed:5.1f}s",
                flush=True,
            )
